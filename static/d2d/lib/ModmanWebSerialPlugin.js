    ...
    static async open() {
        try {
            ...
            const  {start} = useTimeoutFn(()=>{
                ...
                if(WebSerialPlugin.sendCallback ) {
                    WebSerialPlugin.sendCallback(WebSerialPlugin.recBuffer)
                }
                WebSerialPlugin.recBuffer=[];
            },20);
    ...
    static async send(param) {
        ...
        return new Promise(
            async function (reslove) {
                ...
                //合并数据包
                WebSerialPlugin.sendCallback=(vArr)=>{
                    if(WebSerialPlugin.recBuffer.length>=10){
                        reslove(WebSerialPlugin.recBuffer);
